...
Grafana wordt gebruikt om grafieken van data uit influxdb te make, zo wordt het in de opstelling gebruikt om de historieken van de verschillende apparaten te tonen. Grafana kan geïnstalleerd worden via add-ons in home assistant. Om InfluxDB te gebruiken moet het onder configuration toegevoegd worden als een data source, voor influxdb moet je het adres (localhost niet aangeraden), de database naam, user en password instellen. Om ons dashboard te gebruiken kan je onder dashboards -> new dashboard op import klikken en grafana.json uploaden. Het is mogelijk dat de querys van de verschillende panels nog moeten aangepast worden.
